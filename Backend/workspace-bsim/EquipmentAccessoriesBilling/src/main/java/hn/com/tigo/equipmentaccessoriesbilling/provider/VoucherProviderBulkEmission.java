...
@Service
public class VoucherProviderBulkEmission {
    ...
    public VoucherProviderBulkEmission(IBillingService billingService, IInvoiceDetailService invoiceDetailService,
     ...
     * @throws Exception
     */
    public VoucherAppliedData executeAddVoucher(
            final Long id,
            final String user,
            final Long idBranches,
            Long typeApproval,
        ...
        try {
            ...
            // Construir request y consumir servicio
            VoucherType voucherType = this.buildVoucherType(billingModel, products, branchOfficesModel, parameters, user);
            ...
            // Armar DTO con los datos a aplicar luego en Billing
            VoucherAppliedData out = new VoucherAppliedData();
            out.idCompany = voucherType.getIDCompany();
            out.agency = branchOfficesModel.getName();
            out.idBranchOffices = branchOfficesModel.getId();
            out.initialRank = voucherResponseType.getInternal().getRange().getDeinumberStart();
            out.finalRank = voucherResponseType.getInternal().getRange().getDeinumberStop();
            out.idSystem = voucherType.getIDSystem();
            out.numberDei = voucherResponseType.getInternal().getNumberDEI();
            out.deadLine = voucherResponseType.getInternal().getDeadLine().toString();
            out.idReference = voucherResponseType.getInternal().getIDReference();
            out.cai = voucherResponseType.getInternal().getCAI();
            out.idVoucher = voucherResponseType.getInternal().getIDVoucher();
            out.userIssued = user;
            out.dateOfIssue = LocalDateTime.now();
            out.status = 2L;
            out.xml = request;
            out.totalLps = voucherResponseType.getInternal().getLiteralAmount().toString();
            out.totalLpsLetters = voucherResponseType.getInternal().getLiteral();
    ...
    private VoucherType buildVoucherType(BillingModel billingModel, List<InvoiceDetailModel> products,
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (idAmount == 5L) {
                ...
                amountType.setAmount(amountUpdate.setScale(4, RoundingMode.HALF_UP));
            }
