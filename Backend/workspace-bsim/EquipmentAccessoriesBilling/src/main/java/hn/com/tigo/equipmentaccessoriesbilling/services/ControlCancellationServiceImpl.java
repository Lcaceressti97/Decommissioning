...
import hn.com.tigo.equipmentaccessoriesbilling.services.interfaces.IProvisioningInsuranceService;
...
public class ControlCancellationServiceImpl implements IControlCancellationService {
    ...
    @Override
    public Page<ControlCancellationModel> getByTypeCancellation(Pageable pageable, Long typeCancellation) {
        Pageable descendingPageable = PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(),
                Sort.by("id").descending());

        Page<ControlCancellationEntity> entities = controlCancellationRepository
                .findByTypeCancellation(descendingPageable, typeCancellation);

        if (entities.isEmpty()) {
            throw new BadRequestException(Constants.ERROR_NOT_FINDING_BY_STATUS + typeCancellation);
        }

        return entities.map(ControlCancellationEntity::entityToModel);
    }
    ...
    public void add(ControlCancellationModel model) throws Exception {
        ...
        try {
            ...
            validateUserCancellation(billingEntity, model.getUserCreate().toUpperCase(), model, typeWithTaxNumber);
            ...
            if (model.getTypeCancellation() == typeNotIssued) {
                ...
                try {
                    ...
                    VoucherACKType voucherACKType = voucherProvider.cancelVoucher(model.getIdPrefecture(),
                            model.getUserCreate().toUpperCase(), parameters, channelEntity);
