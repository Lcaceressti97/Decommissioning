...
import hn.com.tigo.equipmentaccessoriesbilling.repositories.bsim.IExistencesDao;
...
public class ExistencesController {
	...
	public ResponseEntity<Object> getExistencesViewByFilter(@PathVariable String warehouseId,
			@PathVariable String type, @PathVariable String equipmentLine, HttpServletRequest request) {
	...
	@GetMapping("/all/view/{warehouseId}/{type}/{equipmentLine}")
	public ResponseEntity<Object> getAllExistencesViewByFilter(@PathVariable String warehouseId,
			@PathVariable String type, @PathVariable String equipmentLine, HttpServletRequest request) {
	    if (!warehouseId.matches("\\d+")) {
	        throw new RuntimeException(String.format("The value of '%s' is not valid for warehouse id", warehouseId));
	    }
	    Long warehouseIdLong = Long.parseLong(warehouseId);
		return exceptionHandler.handleExceptions(
				() -> this.existencesDao.getAllExistencesViewByFilter(warehouseIdLong, type, equipmentLine), request);
	}
