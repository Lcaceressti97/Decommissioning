...
import hn.com.tigo.equipmentaccessoriesbilling.models.LoadStockBsimModel;
import hn.com.tigo.equipmentaccessoriesbilling.services.interfaces.IConfigParametersService;

@Service
public class LoadStockBsimService {

	private final IConfigParametersService configParametersService;
	private final ILogsServicesService logsServicesService;

	public LoadStockBsimService(IConfigParametersService configParametersService, ILogsServicesService logsServicesService) {
		super();
		this.configParametersService = configParametersService;
        this.logsServicesService = logsServicesService;
    }

	public LoadStockBsimModel loadStock(String accessToken, String inventoryType, String itemCode, String warehouseCode,
										String subWarehouseCode, List<InvoiceDetailEntity> invoiceDetails, Long reference, String userCreate, ChannelEntity channelEntity) {
		...
		String unloadStockUrl = paramsAuthentication.get("LOAD_STOCK_ENDPOINT");
		String transactionType = paramsAuthentication.get("TRANSACTION_TYPE_LOAD_STOCK");
		...
		Gson gson = new Gson();
		Type loadStockResponseType = new TypeToken<LoadStockBsimModel>() {
		}.getType();
		LoadStockBsimModel loadStockResponse = gson.fromJson(response.getBody(), loadStockResponseType);
		...
		if (channelEntity.getLogs() == 1) {
			...
			logModel.setService("LOAD STOCK SERVICE");
		...
		if (loadStockResponse.getResult_code().endsWith("000")) {
			return loadStockResponse;
		} else {
			throw new BadRequestException("Error load stock: " + loadStockResponse.getResult_message());
		}
