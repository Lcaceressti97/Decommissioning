...
public class MooringServiceImpl implements MooringService{
	...
	public MooringServiceImpl(MooringRepository mooringRepository, IMooringBillingRepository imooringBillingRepository) {
		super();
	...
	public List<MooringModel> findByIdMooringBilling(Long idMooringBilling) {
		...
		if(entities.isEmpty()) {
			throw new BadRequestException(String.format("Error getByIdMooringBilling, Record with id %s is not valid", idMooringBilling));
		}
	...
	public void add(Long id, MooringModel model) {
		...
		if(validateEntity==null) {
			throw new BadRequestException(String.format("Error createMorring, Record with idMooringBilling %s is not valid", id));
		}
	...
	public void addTest(Long id, List<MooringModel> model) {
		...
		for(MooringModel mooringModel : model) {
			...
			if(mooringModel.getIdMooringBilling()<1) {
				throw new BadRequestException(String.format("Error createMorring, Record with idMooringBilling %s is not positive", mooringModel.getIdMooringBilling()));
			}
	...
	public void validateSubcriber(String subcriber) {
		...
		if(!listMorring.isEmpty()) {
			throw new BadRequestException(String.format("The [subscriberId] = %s is already tied and the record cannot be created", subcriber));
		}
		 ...
		 */
		if(!list.isEmpty()) {
			throw new BadRequestException(String.format("The [subscriberId] = %s belongs to an active loan and cannot be created", subcriber));
		}
	...
	public MooringInfoModel getInfoMooringBySubscriber(String subscriber) {
		...
		if(!list.isEmpty()) {
			...
			data.setCost(list.get(0).getPhoneCost());
	        ...
	        Period period = Period.between(currentDate.toLocalDate(), estimatedDate.toLocalDate());
	        int pendingMonths = period.getMonths() + (period.getYears() * 12);

	        data.setPendingMonths(pendingMonths);

			List<MooringEntity> mooringSubs = this.mooringRepository.getMooringByIdMooringBillinfAndStatus(list.get(0).getId(), 1);
			
			List<String> subscribers = new ArrayList<>();
			
			data.setMooring(mooringSubs.isEmpty() ? "N" : "Y");
			...
			MooringBillingEntity father = this.imooringBillingRepository.findById(moorings.get(0).getIdMooringBilling()).orElseThrow(null);
			...
			List<MooringEntity> mooringSubs = this.mooringRepository.getMooringByIdMooringBillinfAndStatus(father.getId(), 1);
