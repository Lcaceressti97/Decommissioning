...
public class HistoricalServiceImpl implements IHistoricalService {
	...
	public GeneralResponseImeiModel addBloqueoImei(AddBloqueoImeiRequestModel request) {
		try {
			...
			Long historicalId = this.historicalRepository.findLastInsertedId(request.getDetallebandit().getEsnImei());
			...
			historicalDetailEntity.setImeiSinUltDigit(request.getDetallebandit().getEsnImei().substring(0, request.getDetallebandit().getEsnImei().length() - 1));
