...
public class ExistencesDaoImpl implements IExistencesDao {
	...
	public List<InvExistencesEntity> getExistencesByFilter(Long warehouseId, Long typeInventoryId) {
		String sql = "SELECT DISTINCT ima.MODEL, ima.DESCRIPTION,ima.CODE,ima.EQUIPMENT_LINE_ID "
				+ "FROM STCINVENTORY.INV_EXISTENCES ie " + "INNER JOIN STCINVENTORY.INV_MODELS_OR_ARTICLES ima "
				+ "ON ie.MODEL_OR_ARTICLE_ID = ima.ID " + "WHERE ie.WAREHOUSE_ID = ? AND ie.TYPE_INVENTORY_ID = ?";
	...
	@Override
	public List<InvExistencesViewEntity> getExistencesViewByFilter(Long warehouseId, String type,
			String equipmentLine) {

		if (!(type.equalsIgnoreCase("catalog") || type.equalsIgnoreCase("inventory"))) {
			throw new RuntimeException(
					String.format("Type value of '%s' is not valid, expected 'catalog' or 'inventory'", type));
		}

		if (!(equipmentLine.equalsIgnoreCase("TEL") || equipmentLine.equalsIgnoreCase("SIM")
				|| equipmentLine.equalsIgnoreCase("ALL"))) {
			throw new RuntimeException(String.format(
					"Equipment Line value of '%s' is not valid, expected 'TEL', 'SIM' or 'ALL'", equipmentLine));
		}

		String sql = "SELECT WAREHOUSE_ID, WAREHOUSE_CODE, INVENTORY_TYPE, LINE_CODE, MODEL_CODE, MODEL, DESCRIPTION, BRAND, QUANTITY "
				+ "FROM STCINVENTORY.VIEW_INV_EXISTENCES " + "WHERE WAREHOUSE_ID = ? ";
