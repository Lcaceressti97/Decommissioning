package hn.com.tigo.simcardinquiry.exceptions;
...
public class ExceptionHandler {
	...
	public ResponseEntity<Object> handleExceptions(ExceptionSupplier supplier, HttpServletRequest request) {
		try {
			...
			return responseBuilder.buildErrorResponse(e, request, HttpStatus.BAD_REQUEST);
		} catch (NotFoundException e) {
			return responseBuilder.buildErrorResponse(e, request, HttpStatus.NOT_FOUND);
		} catch (DataAccessException e) {
			return responseBuilder.buildErrorResponse(e, request, HttpStatus.INTERNAL_SERVER_ERROR);
		} catch (Exception e) {
			return responseBuilder.buildErrorResponse(e, request, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
