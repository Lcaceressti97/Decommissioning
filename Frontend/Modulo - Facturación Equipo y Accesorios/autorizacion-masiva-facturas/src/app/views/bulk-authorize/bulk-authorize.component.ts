...
import { AuthorizeModalComponent } from 'src/app/components/authorize-modal/authorize-modal.component';

@Component({
  selector: 'app-bulk-authorize',
  templateUrl: './bulk-authorize.component.html',
  styleUrls: ['./bulk-authorize.component.css']
})
export class BulkAuthorizeComponent implements OnInit {
   ...
   * - Recarga desde backend getBulkAuthorize()
   */
  async onRefresh() {
    // Form & multiselect
  ...
  openAuthorizeModal() {
    ...
    const modalRef = this.modalService.open(AuthorizeModalComponent, { size: 'lg', backdrop: 'static' });
  ...
  private configParametersById(id: any): Promise<boolean> {
    return new Promise((resolve) => {
      this.bulkAuthorizeService.configParametersById(id).subscribe((response) => {
  ...
  private fetchPage(page = this.currentPage, size = this.pageSize) {
    ...
    if (this.lastSearch) {
      return this.bulkAuthorizeService.searchBulkAuthorize(
        page, size, seller, this.lastSearch.customer, this.lastSearch.customerId
      );
    }
    return this.bulkAuthorizeService.getBulkAuthorize(page, size, seller);
  }
