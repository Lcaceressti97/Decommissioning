...
import { ControlAuthEmission, ControlUserPermissions } from '../models/user-control-permissons';
import { DataVoucherService } from '../models/billing';
 ...
 */
  ...
  /**
   * Método que ingresa los la autorización o emisión
   * 
   * @param page 
   * @param size 
   * @returns 
   */
  postControlAuthEmission(data: ControlAuthEmission, transaction: string): Observable<any> {
    return this.http.post(`${this.localUrl}controlauthemission/add`, data, {
      observe: "response",
      headers: this.headers,
  ...
  */
  ...
  // Métodos para el VoucherService
  postAddVoucher(data: DataVoucherService): Observable<any> {
    return this.http.post(`${this.localUrl}voucher/execute/addVoucher`, data, {
      observe: "response",
      headers: this.headers,
