...
export class ExcelService {
  ...
  /**
   * Método encargado de crear el archivo de excel
   * 
   * @param data 
   * @param sheetName 
   * @param columnNames 
   ...
   */
  generateExcel(data: any[], sheetName: string, columnNames: string[], filterColumnName?: string) {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet(sheetName);
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      this.saveFile(buffer, sheetName + '.xlsx');
    });
  }

  private saveFile(buffer: ArrayBuffer, fileName: string) {
    const data = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    const url = window.URL.createObjectURL(data);
    const link = document.createElement('a');
    link.href = url;
